...
const initDb = async () => {
  try {
    ...
    await pool.query(
      "DROP TABLE IF EXISTS telephone_prefix, direction, users, products, product_size, photos, category, product_category, cart, product_cart, rate"
    );
    ...
    await pool.query(`CREATE TABLE product_cart (
        id INT PRIMARY KEY AUTO_INCREMENT,
        product_size_id INT,
    ...
    await pool.query(`CREATE TABLE rate (
        id INT PRIMARY KEY AUTO_INCREMENT,
        user_id INT,
        rate ENUM('1', '2', '3', '4', '5'),
        coment VARCHAR(225),
